...
// LEARNING destructuring: for more efficiency on manipulating objects, const inventorNames = inventors.map(({ first, last }) => ({ first, last }));
...
let inventorNamedAda: object | undefined = {};
// LEARNING: typescript cannot be object only , because for .find, there is a chance for undefined
...
let isAdultPresent: boolean | null = null;

// Complete the exercise in the space below:
isAdultPresent = devs.some(({ year }) => {
  return new Date().getFullYear() - year >= 18;
});
// LEARNING: new Date() is an object, Date() is a string
...
let isEveryone19OrOlder: boolean | null = null;
...
let commentById: object | undefined = {};
...
totalYearsLived = inventors.reduce((total, { year, passed }) => {
  return total + passed - year;
}, 0);
// Learning, remember to include initial Value
...
travelMethodCounts = travelMethods.reduce((count, method) => {
  if (count[method]) {
    //if method already exists, beacuse Boolean(undefined or empty) = False
    count[method] += 1;
  } else {
    count[method] = 1;
  }
...
// LEARNING, count is initialised as {}, then the callback function runs the if else to return count at every iteration within the array
